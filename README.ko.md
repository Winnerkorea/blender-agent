...
사내 가이드 문서가 없다면 Blender 공식 디자인 기준을 기본값으로 사용합니다.

참고할 공식 문서:

- Human Interface Guidelines: https://developer.blender.org/docs/features/interface/
- Layout 규칙: https://developer.blender.org/docs/features/interface/human_interface_guidelines/layouts/
- Add-on Guidelines: https://developer.blender.org/docs/handbook/extensions/addon_guidelines/
- Add-on 개발 환경: https://developer.blender.org/docs/handbook/extensions/addon_dev_setup/

기본 설계 규칙:

- Blender의 UI 관습을 따르고 별도 스타일을 과하게 만들지 않습니다.
- 자주 쓰고 중요한 기능을 먼저 노출합니다.
- 관련 속성은 heading이나 sub-panel로 묶습니다.
- 선택지가 많은 enum은 dropdown을 사용하고, 빠른 전환이 중요한 경우에만 toggle을
  사용합니다.
- Blender UI 시스템과 어울리지 않는 복잡하거나 장식적인 layout은 피합니다.
- Add-on은 자기 namespace 안에서 동작하도록 만들고, 전역 상태를 불필요하게
  변경하지 않습니다.
